SELECT * FROM thesisv1.salaries2022;

## to check for hispanic names with an accent á é í ó ú 
SELECT *
FROM thesisv1.salaries2022
WHERE Name REGEXP '[áéíúÁÉÍÚ]'
LIMIT 0, 1000;

#replace the ones with á, é, í, ó, ú. 
#lowerletters
UPDATE thesisv1.salaries2022 SET Name = REPLACE(Name, 'á', 'a') WHERE Name LIKE '%á%';
UPDATE thesisv1.salaries2022 SET Name = REPLACE(Name, 'ó', 'o') WHERE Name LIKE '%ó%';
UPDATE thesisv1.salaries2022 SET Name = REPLACE(Name, 'é', 'e') WHERE Name LIKE '%é%';
UPDATE thesisv1.salaries2022 SET Name = REPLACE(Name, 'í', 'i') WHERE Name LIKE '%í%';
UPDATE thesisv1.salaries2022 SET Name = REPLACE(Name, 'ú', 'u') WHERE Name LIKE '%ú%';
...
##to keep batters 
SELECT * FROM thesisv1.salaries2022;
DELETE FROM thesisv1.salaries2022
WHERE IP > AB;

#drop the IP and AB columns after 
ALTER TABLE thesisv1.salaries2022
DROP COLUMN IP,
DROP COLUMN AB;
SELECT * FROM thesisv1.salaries2022;

#keep only the year in date 
UPDATE thesisv1.salaries2022
